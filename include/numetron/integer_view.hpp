...
public:
    ...
    template <LimbT ... ls>
    basic_integer_view(ct::limbs<LimbT, ls...>, int sign = 1) noexcept
        : limbs_{ ct::limbs<LimbT, ls...>::data }, ctl_{ sizeof...(ls), sign }
    {}
    ...
    inline explicit operator bool() const noexcept
    {
        ...
        if (ctl_.skip_bits) {
            if (*limbs & last_significand_limb_mask()) return true;
        } else {
    ...
    inline LimbT at(size_t index) const noexcept
    {
        if (index >= size()) [[unlikely]] return LimbT{ 0 };
            ...
            return limbs_[index] & last_significand_limb_mask();
        }
        return limbs_[index];
    }
    ...
    inline OutputIteratorT copy_to(OutputIteratorT oi) noexcept
    {
        if (!ctl_.skip_bits) {
            ...
            *r = *last_limb_ptr & last_significand_limb_mask();
            return std::move(++r);
        }
    }
    ...
    inline decltype(auto) with_limbs(FunctorT const& ftor, AllocatorT && alloc = AllocatorT{}) const
    {
        ...
        if (!ctl_.skip_bits) {
            ...
            std::memcpy(buff, limbs_, size() * sizeof(LimbT));
            buff[size() - 1] &= last_significand_limb_mask();
            ...
            NUMETRON_SCOPE_EXIT([this, &alloc, buff] { alloc_traits_t::deallocate(alloc, buff, size()); });
            std::memcpy(buff, limbs_, size() * sizeof(LimbT));
            buff[size() - 1] &= last_significand_limb_mask();
            return ftor(std::span<const_limb_t>{ buff, size() }, sgn());
        }
