...
inline LimbT uadd_partial_unchecked(LimbT const*& ub, LimbT const* vb, LimbT const* ve, LimbT *& rb)
{
    LimbT c = 0;
    for (; vb != ve; ++ub, ++vb, ++rb) {
        std::tie(c, *rb) = numetron::arithmetic::uadd1c(*ub, *vb, c);
    }
    return c;
}
...
inline LimbT uadd_partial_unchecked(LimbT *& ub, LimbT const* vb, LimbT const* ve)
{
    LimbT c = 0;
    for (; vb != ve; ++ub, ++vb) {
        std::tie(c, *ub) = numetron::arithmetic::uadd1c(*ub, *vb, c);
    }
    return c;
}
...
inline LimbT uadd_unchecked(LimbT uh, LimbT const* ub, LimbT const* ue, LimbT vh, LimbT const* vb, LimbT const* ve, LimbT*& rb)
{
    LimbT c = uadd_partial_unchecked(ub, vb, ve, rb);
    if (ub != ue) {
        std::tie(c, *rb++) = numetron::arithmetic::uadd1c(*ub++, vh, c);
        for (; ub != ue; ++ub, ++rb) {
            std::tie(c, *rb) = numetron::arithmetic::uadd1(*ub, c);
        }
        std::tie(c, *rb++) = numetron::arithmetic::uadd1(uh, c);
    } else {
        std::tie(c, *rb++) = numetron::arithmetic::uadd1c(uh, vh, c);
    }
    return c;
}
...
//        std::tie(c, urh) = numetron::arithmetic::uadd1c(urh, v.front(), c);
//    } else {
//        std::tie(c, urh) = numetron::arithmetic::uadd1(urh, c);
//    }
//    return c;
//}
...
//    if (vlsp.size() < v.size()) {
//        assert(vlsp.size() + 1 == v.size());
//        std::tie(c, uh) = numetron::arithmetic::usub1c(uh, v.back(), c);
//    } else {
//        std::tie(c, uh) = numetron::arithmetic::usub1(uh, c);
//    }
//    return c;
//}
...
//    std::tie(c, uh) = numetron::arithmetic::usub1c(uh, vh, c);
//    std::tie(c, uhh) = numetron::arithmetic::usub1(uhh, c);
//    return c;
//}
...
{
    for (LimbT & e : u) {
        auto [h, l] = numetron::arithmetic::umul1(e, v);
        l += cl;
        cl = (l < cl) + h;
        e = l;
    }
...
inline LimbT umul1_add(std::span<const LimbT> u, LimbT v, LimbT* p, LimbT cl = 0) noexcept
{
    for (LimbT const& e : u) {
        auto [h, l] = numetron::arithmetic::umul1(e, v);
        auto [ph, pl] = numetron::arithmetic::uadd1c(l, *p, cl);
        cl = h + ph;
        *p++ = pl;
    }
    return cl;
...
inline LimbT umul1_add(LimbT uhh, LimbT uh, std::span<const LimbT> ul, LimbT v, LimbT* p, LimbT cl = 0) noexcept
{
    ...
    p += ul.size();
    auto [h, ph] = numetron::arithmetic::umul1<LimbT>(uh, v);
    auto [rc, rh] = numetron::arithmetic::uadd1c(ph, *p, pcl);
    *p++ = rh;
    auto [hh, phh] = numetron::arithmetic::umul1<LimbT>(uhh, v);
    auto [rch, rhh] = numetron::arithmetic::uadd1c(phh, *p, rc);
    *p++ = rh;
    return rch;
}
            ...
            for (;;)
            {
                ...
                if (uc) {
                    do {
                        ...
                        std::tie(uc, *puh) = numetron::arithmetic::uadd1c(*puh, dh, uc);
            ...
            for (;;)
            {
                ...
                if (uc) {
                    do {
                        ...
                        std::tie(uc, *puh) = numetron::arithmetic::uadd1c(*puh, d.back(), uc);
                        std::tie(uc, *puhh) = numetron::arithmetic::uadd1(*puhh, uc);
                    } while (!uc);
                }
                break;
