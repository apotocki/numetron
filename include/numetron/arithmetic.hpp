...
inline constexpr unsigned int count_leading_zeros(T x) noexcept
{
    if (std::is_constant_evaluated()) {
        ...
        if constexpr (sizeof(T) == 2) {
            ...
            return __lzcnt(x);
        } else if constexpr (sizeof(T) == 8 && sizeof(void*) == 8) {
...
inline constexpr T uadd1ca(T a, T b, T& ca) noexcept
{
    ...
    ca += __builtin_add_overflow(a, b, &a);
    return a;
#endif
#if (defined(__GNUC__) || defined(__clang__)) && defined(__x86_64__)
        __asm__(
...
inline constexpr std::pair<unsigned char, T> uadd1(T a, T b) noexcept
{
    ...
    if (!std::is_constant_evaluated()) {
        if constexpr (bsz == 64) {
            unsigned char carry = __builtin_add_overflow(a, b, &a);
            return { carry, a };
        }
    }
#endif
...
//#endif
    ...
    if (!std::is_constant_evaluated()) {
        if constexpr (bsz == 64) {
            unsigned char carry = __builtin_add_overflow(a, b, &a);
            carry += __builtin_add_overflow(a, c, &a);
            return { carry, a };
        }
    }
#endif
