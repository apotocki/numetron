...
#include "limbs_from_decimal_string.hpp"
#include "integer_view.hpp"

#include "basic_integer.hpp"
#include "decimal_view.hpp"
...
struct decimal_holder : AllocatorT
{
    ...
    static_assert(ExponentBitCount + intptr_t(significand_limb_count_bits) + 2 < intptr_t(limb_bits));
    ...
    static constexpr LimbT exp_sign_mask = has_exp_sign_bit ? LimbT(1) << (limb_bits - 3 - significand_limb_count_bits) : 0;
    static constexpr size_t exp_value_bit_offset = limb_bits - 2 - exp_value_bits - (significand_limb_count_bits + (has_exp_sign_bit ? 1 : 0));
    static constexpr LimbT exp_bits_mask = exp_sign_mask | (max_inplace_exp_value << exp_value_bit_offset);

    static constexpr LimbT last_limb_mask_bits = (has_exp_sign_bit ? 1 : 0) + exp_value_bits + significand_limb_count_bits + 2; // takes into account in_place_mask, sign_mask, limb_count_mask
    ...
    [[nodiscard]] inline std::tuple<DataT const*, LimbT const*> allocated_data_and_limbs() const noexcept
    {
        DataT const* ldata;
        if constexpr (!overwritten_limb_flag) {
            ldata = *std::launder(reinterpret_cast<DataT const* const*>(inplace_limbs_));
        } else {
    ...
    void set_exponent(basic_integer_view<LimbT> exponent)
    {
        ...
        if (is_inplaced(ctl)) {
            ...
            if (can_be_inplaced(exponent)) {
                ...
                if (inplaced_size(ctl) == N) *(lastlimb - 1) &= significand_limb_count_mask;
                if (!esz) {
...
class basic_decimal
{
    static_assert(std::numeric_limits<LimbT>::digits > ExponentBitCount); // must fit exponent in LimbT
...
public:
    ...
    inline explicit operator T() const
    {
        int_least64_t eval = exponent_as<int_least64_t>();
        if (eval >= 0) {
            if (std::numeric_limits<T>::digits10 >= eval && aholder_.template is_fit_significand<T>()) {
                using UT = std::make_unsigned_t<T>;

                UT value = (UT)aholder_.significand().abs();
                UT emultiplier = numetron::arithmetic::ipow<UT>(10, eval);
            
                auto [h, l] = numetron::arithmetic::umul1(value, emultiplier);
                if (!h) {
            ...
            uint_least64_t positive_eval = static_cast<uint_least64_t >(-eval);
            ...
            constexpr uint64_t stepdivider = numetron::arithmetic::ipow<LimbT>(10, step);
            
            while (step < positive_eval) {
                sig /= stepdivider;
                if (!sig) return 0;
                positive_eval -= step;
            }
            uint64_t lastdivider = numetron::arithmetic::ipow<LimbT>(10, positive_eval);
            sig /= lastdivider;
            return (T)sig;
        }
